...
descompcta_zip <- function(arq_zip, dir_destino)
{
  tryCatch({
    ...
    if (file.exists(arq_zip)){
      ...
      cat(paste(format(Sys.time(), "%Y/%m/%d %H:%M:%S"), paste("Arquivo não encontrado", basename(arq_zip), sep = " "), sep = ' - '), file=logFile, append=TRUE, sep = "\n")
    }
...
processa_dia <- function(dat)
{
  tryCatch({
    ...
    #tem arquivos zipados que os .json estão na pasta tmp
    ...
    #e tem arquivos zipados que os .json estão na raiz
